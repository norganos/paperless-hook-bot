...
class DefaultPaperlessContextFactory: PaperlessContextFactory {
    ...
    private val cache = mutableMapOf<PaperlessConnector, CacheValue>()
    override suspend fun createContext(connector: PaperlessConnector): PaperlessContext {
        return cache[connector]
                ?.takeIf { it.ttl > System.currentTimeMillis() }
                ?.context
            ?: createNewContext(connector)
                .also { cache[connector] = it }
                .context
    }

    private suspend fun createNewContext(connector: PaperlessConnector): CacheValue {
        return CacheValue(
            context = PaperlessContext(
